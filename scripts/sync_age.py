...
async def main():
    ...
    # Stats before
    age_nodes = await conn.fetchval("SELECT * FROM cypher('ecodb_graph', $$MATCH (n) RETURN count(n)$$) AS (c agtype)")
    age_edges = await conn.fetchval("SELECT * FROM cypher('ecodb_graph', $$MATCH ()-[r]->() RETURN count(r)$$) AS (c agtype)")
    ...
    print("Vaciando AGE...")
    await conn.execute("SELECT * FROM cypher('ecodb_graph', $$MATCH ()-[r]->() DELETE r$$) AS (d agtype)")
    await conn.execute("SELECT * FROM cypher('ecodb_graph', $$MATCH (n) DELETE n$$) AS (d agtype)")

    check = await conn.fetchval("SELECT * FROM cypher('ecodb_graph', $$MATCH (n) RETURN count(n)$$) AS (c agtype)")
    ...
    for r in nodes:
        ...
        await conn.execute(
            "SELECT * FROM cypher('ecodb_graph', $$CREATE (:Entity {name: $name, sql_id: $sql_id})$$, $1::agtype) AS (d agtype)",
            params,
        )
    ...
    # Stats after
    age_nodes_after = await conn.fetchval("SELECT * FROM cypher('ecodb_graph', $$MATCH (n) RETURN count(n)$$) AS (c agtype)")
    age_edges_after = await conn.fetchval("SELECT * FROM cypher('ecodb_graph', $$MATCH ()-[r]->() RETURN count(r)$$) AS (c agtype)")
